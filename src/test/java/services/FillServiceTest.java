...
public class FillServiceTest {
     ...
     * insert a user in the database so that user can be logged in the tests
     */
    @BeforeEach
    public void setUp() {
        db.createTables();
    ...
    public void FillDefault() {
        ...
        try {
            Connection conn = db.openConnection();
            String sql = "SELECT count(*) as rows FROM Persons WHERE associatedUsername=\"username\"";
            PreparedStatement stmt = conn.prepareStatement(sql);
            numPeopleGenerated = stmt.executeQuery().getInt("rows");
            sql = "SELECT count(*) as rows FROM Events WHERE associatedUsername=\"username\"";
    ...
    public void FillSixGens() {
        ...
        FillRequest request = new FillRequest(6,"username");
        Result result = new FillService().fill(request);
    ...
    public void FillZeroGens() {
        ...
        FillRequest request = new FillRequest(0,"username");
        Result result = new FillService().fill(request);

        try {
            Connection conn = db.openConnection();
            String sql = "SELECT count(*) as rows FROM Persons WHERE associatedUsername=\"username\"";
            PreparedStatement stmt = conn.prepareStatement(sql);        //Problem with this line
            numPeopleGenerated = stmt.executeQuery().getInt("rows");
            sql = "SELECT count(*) as rows FROM Events WHERE associatedUsername=\"username\"";
            stmt = conn.prepareStatement(sql);
            numEventsGenerated = stmt.executeQuery().getInt("rows");
    ...
    public void FillNegativeGens() {
        ...
        FillRequest request = new FillRequest(-1,"username");
        Result result = new FillService().fill(request);
