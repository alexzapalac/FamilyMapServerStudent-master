...
public class FillService {
    ...
    public Result fill(FillRequest req) {
        ...
        if (req.getGenerations() < 0) {
            return new Result("Generations must be a nonnegative integer", false);
        }
        ...
        try {
            ...
            if(!db.getUserDao().find("username", req.getUsername()).isEmpty()) {
                ...
                return new Result("The user you are trying to fill for has not been registered", false);
            }
            ...
            result = new Result("Successfully added " + Integer.toString(peopleAdded)+
                    " persons and " + Integer.toString(eventsAdded) + " events to the database", true);
            commit = true;
    ...
    private void generateBirthEvent(Person p, int gen, Database db) {
        ...
        Event birthEvent = new Event(birthID, p.getAssociatedUsername(), p.getPersonID(),
    ...
    private void generateMarriageEvents(Person p, Location marriageLoc, int gen, Database db) {
        ...
        Event motherMarriageEvent = new Event(marriageID, p.getAssociatedUsername(), p.getPersonID(),
    ...
    private void generateOtherEvents(Person p, int gen, Database db) {
        ...
        Event otherEvent = new Event(eventID, p.getAssociatedUsername(), p.getPersonID(),
