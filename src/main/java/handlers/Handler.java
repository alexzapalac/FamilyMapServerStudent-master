        ...
        try {
            ...
            if (requestMethodOK()) {
                ...
                if (authTokenRequired() && !authTokenWasProvided(exchange)) {
                    result = new Result("No auth token was provided", false);
                } else if (authTokenRequired() && !authTokenExists(exchange)) {
                    result = new Result("Provided auth token is not registered", false);
                } else {
                ...
                result = new Result("Incorrect GET / POST value", false);
                returnString = s.serialize(result);
            }
        } catch (Exception e) {
